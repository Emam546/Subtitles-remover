...
export default function MapData({ video, title, clippedData }: Props) {
  ...
  return (
    <tr>
      <td className="w px-4 py-2.5 border border-[#ddd] w-full">
        {video.text.str}
        {video.text.label && (
      ...
      <td className="px-4 py-2.5 border border-[#ddd] w-auto min-w-[5rem]">
        <p className="whitespace-nowrap">
          {`${formatBytes(parseInt(`${video.size}`), 0)}`}
        </p>
      </td>
      <td className="button-column px-4 py-2.5 border border-[#ddd] w-auto">
        <DownloadButton
          shrink
          text="Download"
          onClick={() => {
