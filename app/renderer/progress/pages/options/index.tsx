    ...
    <div>
      <div>
        ...
        <CheckLabel
          id={"show-dialog"}
          onChange={(e) => {
            dispatch(OptionsActions.setShowDialog(e.target.checked));
          }}
          checked={option}
        >
      ...
      <hr className="my-2" />
      <div>
        <h2 aria-hidden={!option} className="aria-hidden:text-gray-400">
          These options are unavailable &quot;Show download complete
          dialog&quot; is turned on
        </h2>
        <div aria-disabled={option} className="aria-disabled:text-gray-500">
          <CheckLabel
            onChange={(e) => {
              dispatch(
            ...
            disabled={option}
          >
            <p>Exit subtitles manager when done</p>
          </CheckLabel>
                ...
                onChange={(e) => {
                  dispatch(
                    OptionsActions.setShutDownState(
                      e.currentTarget.value as ShutDownType
                    )
                  );
                }}
                disabled={option}
              >
                <option value={ShutDownType.SHUTDOWN}>Shut down</option>
                <option value={ShutDownType.SLEEP}>Sleep</option>
              </select>
            </div>
          </div>
        </div>
