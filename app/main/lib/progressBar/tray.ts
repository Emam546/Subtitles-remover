...
export class DownloadTray extends Tray {
  ...
  constructor(...options: ConstructorParameters<typeof Tray>) {
    ...
    this.setToolTip("Subtitles Remover Downloads");
  ...
  static addWindow(window: BrowserWindow) {
    window.addListener("close", () => {
      if (this.HidedWindows[window.id]) delete this.HidedWindows[window.id];
      this.construct();
    });
    ...
    const contextMenu = Menu.buildFromTemplate([
      {
        ...
        click: () => {
          objectValues(this.HidedWindows).forEach((window) => window.show());
        },
      },
      { type: "separator" },
